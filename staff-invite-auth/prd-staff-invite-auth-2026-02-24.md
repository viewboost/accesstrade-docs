...
- ~~Bulk invite (nhiều nhân viên cùng lúc) — Phase 3, optional~~ ✅ **Đã implement (2026-02-25)**
...
**Acceptance Criteria:**
- [x] Nút ✉️ (mail icon) xuất hiện trong cột actions cho nhân viên có status Pending và chưa kích hoạt
- [x] Khi gửi lại, token cũ bị vô hiệu hóa, token mới được tạo với expiry 48h mới
- [x] Email mới được gửi với link mới
- [x] Admin thấy xác nhận gửi thành công + link được copy vào clipboard

**API:** `POST /staffs/:id/resend-invite`

**Dependencies:** FR-001, FR-002

---

#### FR-003b: Mời hàng loạt ✅

**Priority:** Should Have
**Status:** Implemented (2026-02-25)

**Description:**
Admin có thể mời nhiều nhân viên cùng lúc trong một thao tác.

**Acceptance Criteria:**
- [x] Nút "Mời hàng loạt" trên trang Staff
- [x] Modal cho phép nhập tối đa 50 nhân viên với email, tên, vai trò, đối tác
- [x] Hệ thống xử lý song song (goroutines), không block
- [x] Kết quả hiển thị chi tiết: từng email thành công/thất bại với lý do
- [x] Có thể copy invite link cho từng người thành công

**API:** `POST /staffs/bulk-invite` — payload: `{ items: [{email, name, partner?, role?}] }`

**Response:**
```json
{
  "results": [{"email": "...", "inviteLink": "...", "error": "..."}],
  "success": 3,
  "failed": 1
}
```
