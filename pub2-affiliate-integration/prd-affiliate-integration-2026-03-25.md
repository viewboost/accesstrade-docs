...
- [ ] Hiển thị trạng thái tham gia cho influencer:
  - `PENDING`: Banner vàng "Yêu cầu tham gia đang được xử lý" — disable nút tạo link, hiển thị "Thử lại sau X giờ" (countdown dựa vào `retryAfter` từ API)
  - `APPROVED`: "Đã được duyệt" — enable nút tạo link
  - `REJECTED`: Banner đỏ "Bạn không đủ điều kiện tham gia chiến dịch này" — disable nút tạo link, hiển thị "Thử lại sau X ngày" (countdown dựa vào `retryAfter` từ API)
- [ ] **Retry mechanism cho PENDING/REJECTED:**
  - Backend tính `canRetry` (bool) và `retryAfter` (timestamp, optional) dựa vào `updatedAt` + config
  - Thời gian chờ retry cấu hình qua ENV: `AFFILIATE_RETRY_PENDING_SECONDS` (default: 86400 = 24h), `AFFILIATE_RETRY_REJECTED_SECONDS` (default: 1209600 = 14 ngày)
  - Khi `canRetry = true` → hiện nút "Thử lại" cho phép gọi lại Pub2 API
  - Khi `canRetry = false` → hiện text "Thử lại sau X giờ/ngày" (tính từ `retryAfter`)
  - Backend cho phép gọi lại Pub2 khi hết thời gian chờ, cập nhật `contractStatus` + `updatedAt` theo response mới
  - API `GET /affiliate/campaigns/:id/contract` trả thêm `canRetry` + `retryAfter` + `updatedAt`
