...
**Acceptance Criteria:**
- [ ] Admin có thể tạo affiliate campaign với các trường: title, description, banner image, category, commission type (CPA/CPL/CPI), commission rate, pub2_campaign_id, pub2_campaign_url (original_url), status, start_date, end_date
- [ ] Admin có thể chỉnh sửa và cập nhật campaign
...
- [ ] Quan hệ many-to-many: bảng mapping `campaign_affiliate_mappings` với `campaign_id` + `affiliate_campaign_id`
- [ ] Admin có thể gỡ liên kết (unlink)
- [ ] Hiển thị danh sách affiliate campaigns đã liên kết trong trang chi tiết campaign (admin)
- [ ] Hiển thị danh sách campaigns đã liên kết trong trang chi tiết affiliate campaign (admin)
- [ ] Chỉ cho phép liên kết affiliate campaign có status = active
...
**Acceptance Criteria:**
- [ ] Trong trang chi tiết campaign hiện tại, hiển thị section "Affiliate Campaigns" nếu campaign có liên kết affiliate
- [ ] Hiển thị thông tin affiliate campaign: title, description, commission type & rate, banner
...
- [ ] CTA "Tạo link affiliate" visible nhưng khi bấm:
  - Nếu chưa link AccessTrade → show popup yêu cầu liên kết trước
  - Nếu đã link → thực hiện tạo link (FR-005)
- [ ] Nếu có nhiều affiliate campaigns liên kết → hiển thị danh sách, cho phép chọn
...
- [ ] `GET /admin/affiliate-campaigns` — Danh sách campaigns (filter, search, pagination)
- [ ] `GET /admin/affiliate-campaigns/:id` — Chi tiết campaign
- [ ] `PUT /admin/affiliate-campaigns/:id` — Cập nhật campaign
- [ ] `PATCH /admin/affiliate-campaigns/:id/status` — Thay đổi status
- [ ] Validate: pub2_campaign_id unique, required fields
