# Product Requirements Document: Creator Profile cho Ambassador — Đăng ký kênh & Làm giàu dữ liệu (Enrich)
...
PRD này định nghĩa yêu cầu chức năng (FR) và phi chức năng (NFR) cho việc **nhân bản chức năng creator-profile phía creator từ T-Fluencers (Techcombank) sang Ambassador**: creator tự đăng ký & xác minh kênh social, khai báo hồ sơ, hệ thống tự làm giàu dữ liệu (enrich) qua AT-Core, và creator theo dõi trạng thái kênh.
...
- Nguồn tham chiếu (T-Fluencers): chức năng hồ sơ creator trong codebase `techcombank/backend` — luồng `LinkUserSocial`, collections `user-socials` / `user-social-partners` / `influencer-profiles` *(tên gốc bên T-Fluencers; sang Ambassador đổi thành `creator-profiles`)*, webhook enrichment, AT-Core client.
...
Ambassador dùng chung nền tảng công nghệ với T-Fluencers nhưng phần hồ sơ creator mới ở mức **cơ bản**: lưu link/tài khoản creator khai báo, **không** xác minh quyền sở hữu chặt, **không** có dữ liệu hồ sơ đầy đủ (follower thật, engagement, score, nhân khẩu học), creator **không** thấy được trạng thái kênh.
...
1. **Đạt ngang bằng T-Fluencers về dữ liệu hồ sơ creator** — Ambassador có dữ liệu kênh đầy đủ và đáng tin (follower thật, engagement, score, nhân khẩu học) để chọn creator, phân nhóm chiến dịch, báo cáo cho thương hiệu.
2. **Giảm gian lận, làm sạch dữ liệu** — xác minh quyền sở hữu kênh trước khi ghi nhận, thay vì tin tưởng link creator tự khai.
3. **Không gây gián đoạn** — luồng đăng bài hiện tại của creator Ambassador giữ nguyên 100%; hồ sơ creator là module song song, không phải tiền đề mới của đăng bài.
...
**Description:**
Sau khi liên kết kênh thành công, creator nhập thông tin định tính về kênh và khán giả vào hồ sơ creator (lưu ở `creator-profiles`).

**Acceptance Criteria:**
- [ ] Creator nhập được: lĩnh vực nội dung chính (categories, tối đa 3), lĩnh vực phụ (subCategories, tối đa 10), ngôn ngữ nội dung, độ tuổi khán giả, giới tính.
- [ ] Ràng buộc số lượng (categories ≤ 3, subCategories ≤ 10) được enforce; vượt quá bị từ chối với thông báo rõ.
- [ ] Dữ liệu wizard lưu vào bản ghi `creator-profile` gắn với `user` và `userSocial` tương ứng.
- [ ] Wizard **không bắt buộc** để hoàn tất liên kết kênh (kênh vẫn tồn tại nếu creator bỏ qua wizard) — trừ khi chốt khác ở tech-spec.

**Dependencies:** FR-001, FR-009 (creator-profile model)
...
**Description:**
Hệ thống cung cấp endpoint nhận webhook callback từ AT-Core khi job enrich xong, rồi lưu kết quả vào `user-social` và `creator-profile`.

**Acceptance Criteria:**
- [ ] Có endpoint public nhận webhook (ví dụ `POST /creator-profiles/webhook/enrichment`), được bảo vệ bằng cơ chế xác thực webhook AT-Core (không phải login creator).
- [ ] Khi `status = completed`: cập nhật `user-social.stats` (followers, views, likes, videoCount, engagementRate, avgViews), set `isEnriched = true`, `atCoreStatus = synced`, `atCoreSyncedAt = now`; đồng thời upsert `creator-profile` với enriched data (scoreTotal, scoreCategory, handle, platform, avatarUrl, categories, isVerified, avgViews, totalViews, totalLikes, tracking.enrichedAt).
- [ ] Khi `status = failed`: set `atCoreStatus = failed`, ghi `tracking.errorCode` / `tracking.errorMsg` vào creator-profile.
...
### FR-009: Data model `creator-profile`

**Priority:** Must Have

**Description:**
Tạo collection/model `creator-profile` lưu hồ sơ creator: phần creator khai báo (wizard) + phần enriched từ AT-Core.

**Acceptance Criteria:**
- [ ] Có model `creator-profile` với các nhóm trường: định danh (`user`, `userSocial`, `profileId`, `handle`, `platform`), creator khai báo (`displayName`, `categories`, `subCategories`, `contentLanguage`, `audienceAge`, `gender`), enriched (`scoreTotal`, `scoreCategory`, `avatarUrl`, `description`, `isVerified`, `avgViews`, `totalViews`, `totalLikes`), và `tracking` (enrichedAt/errorCode/errorMsg).
- [ ] Dual-write nhất quán: stats → `user-social`, enriched/score → `creator-profile` (như luồng T-Fluencers).
...
### FR-011: Creator theo dõi số liệu kênh

**Priority:** Must Have

**Description:**
Trên cổng creator, creator xem được danh sách kênh đã đăng ký, nhóm theo nền tảng, kèm số liệu sau enrich.

**Acceptance Criteria:**
- [ ] Trang hồ sơ hiển thị danh sách kênh nhóm theo nền tảng (TikTok/YouTube/Facebook/Instagram).
- [ ] Sau khi enrich về, kênh hiển thị số liệu (followers, views, likes, engagement...) và tự cập nhật khi dữ liệu thay đổi.
...
**Description:**
Việc port creator-profile **không** được thay đổi điều kiện gắn hồ sơ social khi đăng bài. Luồng submit content của Ambassador giữ **y như hiện tại**: nguồn nào đang require chọn social thì vẫn require, nguồn nào không thì vẫn không. Tuyệt đối không bê ràng buộc "mọi nguồn đều bắt buộc chọn hồ sơ đã duyệt" của T-Fluencers.
...
- [ ] Gỡ kênh không làm hỏng dữ liệu `creator-profile`/`user-social-partner` liên quan (xử lý nhất quán, có quy ước rõ ở tech-spec).
...
### EPIC-001: Nền tảng dữ liệu hồ sơ creator (data layer)

**Description:**
Dựng/mở rộng các model và migration làm nền cho toàn bộ feature: mở rộng `user-social`, tạo `user-social-partner`, `creator-profile`.
...
**Description:**
Wizard creator nhập thông tin định tính, lưu vào creator-profile.
...
### EPIC-005: Hiển thị số liệu kênh cho creator (UI)

**Description:**
Cổng creator hiển thị danh sách kênh và số liệu enrich.
...
- **EPIC-005:** "Là một creator, tôi muốn thấy số liệu kênh của mình ra sao, để biết hiệu quả kênh."
...
- **Creator (Ambassador):** người dùng cuối, đăng ký kênh, khai báo hồ sơ, đăng bài tham gia chiến dịch. Đối tượng chính của PRD này.
...
2. **Khai báo hồ sơ:** Sau liên kết → wizard nhập categories/khán giả → lưu creator-profile.
...
4. **Quy ước unlink:** gỡ kênh thì xử lý creator-profile/user-social-partner liên quan thế nào (giữ/ẩn/xóa) — chốt ở tech-spec.
...
| EPIC-005 | Hiển thị số liệu kênh creator | FR-011 | 3–5 |
