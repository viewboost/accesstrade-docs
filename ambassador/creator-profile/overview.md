# Tính năng Hồ sơ Creator cho Ambassador — Đăng ký kênh & Làm giàu dữ liệu (Enrich)
...
**Phạm vi:** Cổng creator của Ambassador (web) — phần hồ sơ creator; **không** bao gồm màn hình admin xem/duyệt dữ liệu trên dashboard
...
> **Yêu cầu gốc:** clone toàn bộ chức năng hồ sơ (ở T-Fluencers gọi là `influencer-profile`, sang Ambassador đặt là **`creator-profile`**) từ Techcombank sang Ambassador, bao gồm luồng đăng ký kênh (creator tự nhập thông tin) và luồng làm giàu dữ liệu hồ sơ — **không** bao gồm phần admin xem dữ liệu trên dashboard.
...
Nhân bản từ T-Fluencers sang Ambassador **bốn mảnh ghép** của hồ sơ creator:
...
Thông tin này được lưu kèm hồ sơ creator, bổ sung cho dữ liệu hệ thống tự thu thập.
...
Khi nhân bản creator-profile, ta **giữ nguyên cách Ambassador đang xử lý**: nguồn nào đang require chọn social thì vẫn require, nguồn nào không require thì vẫn không. **Tuyệt đối không** thay luật này bằng ràng buộc "mọi nguồn đều phải chọn hồ sơ đã duyệt" của T-Fluencers. Lý do:
...
- Hồ sơ creator và việc đăng bài là **hai lớp độc lập**: hồ sơ là "lớp dữ liệu làm giàu", còn đăng bài là "lớp hoạt động" với luật riêng đã có. Việc làm giàu hồ sơ **không** được thay đổi điều kiện đăng bài.
...
- Hồ sơ creator được xây như một **module song song**, không phải tiền đề bắt buộc mới của đăng bài.
...
- **Nguồn tham chiếu (T-Fluencers):** chức năng `influencer-profile` trong codebase Techcombank — luồng `LinkUserSocial`, kho dữ liệu `user-socials` / `user-social-partners` / `influencer-profiles`, webhook làm giàu từ AT-Core, và tác vụ vớt job treo. *(Ở Ambassador, feature đặt tên `creator-profile` và collection tương ứng là `creator-profiles`.)*
