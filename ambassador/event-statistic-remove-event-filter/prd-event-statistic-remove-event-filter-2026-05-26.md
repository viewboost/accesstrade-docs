# Product Requirements Document: Bỏ lọc thống kê theo event ở API public `/events/statistic` (Ambassador)
...
PRD này định nghĩa yêu cầu chức năng và phi chức năng cho việc **gỡ bỏ khả năng lọc thống kê theo từng `event`** trên API public `GET /events/statistic` của Ambassador. Đây là source of truth cho phần "what" và "how-well"; phần "how" (kỹ thuật) đã có trong tech-spec đi kèm.

> Cùng lớp vấn đề với vcreator (Gen-Green) và T-Fluencers (3 sản phẩm fork cùng nguồn). Fix này nên được đồng bộ cả 3 sản phẩm.

**Related Documents:**
- Overview: [`overview.md`](./overview.md)
- Tech Spec: [`tech-spec.md`](./tech-spec.md)
- Bản cùng họ: [`../../gen-green/event-statistic-remove-event-filter/`](../../gen-green/event-statistic-remove-event-filter/), [`../../t-fluencers/event-statistic-remove-event-filter/`](../../t-fluencers/event-statistic-remove-event-filter/)
- Code reference (BE — `ambassabor/backend`): `pkg/public/handler/event.go:88-119`, `pkg/public/service/event.go:88-175`, `pkg/public/router/event.go:22`

---

## Executive Summary

API public `GET /events/statistic` nhận query param `event` (Event ID) cùng `partner`, `category`, `fromAt/toAt`, và trả về thống kê **của riêng event đó**: số video được duyệt (`totalContent`), tổng view (`totalView`), tổng hoa hồng (`totalCommission`), số user có content (`totalUserWithContent`).

Endpoint này là **public** (route không có `RequiredLogin`; middleware `auth.Auth` chỉ parse JWT optional, không enforce API key cứng). Bất kỳ ai biết một `event_id` đều gọi được và đọc số liệu business chi tiết của campaign đó — có thể enumerate nhiều `event_id` để gom số liệu toàn bộ. Đây là **rò rỉ thông tin business nhạy cảm**.

Mục tiêu: gỡ tác dụng của param `event` ở endpoint public này → public user chỉ thấy số liệu tổng hợp (theo domain/partner/category), không suy ra được số của một event cụ thể. API admin (nội bộ, có auth) **không** thay đổi.
...
2. **Không gây gián đoạn** — frontend Ambassador và đối tác hiện tại vẫn hoạt động bình thường; không cần deploy đồng bộ FE+BE.
...
- Trang home Ambassador vẫn render block thống kê (0 báo lỗi).
...
**Description:**
Khi xử lý `GET /events/statistic`, hệ thống **không** được dùng query param `event` để giới hạn phạm vi thống kê. Kết quả trả về luôn là số liệu tổng hợp theo domain (và `partner`/`category`/`fromAt-toAt` nếu có), bất kể client có gửi `event` hay không.
...
### FR-002: Giữ nguyên các filter hợp lệ (`partner`, `category`, `fromAt/toAt`)

**Priority:** Must Have

**Description:**
Các param `partner`, `category`, `fromAt/toAt` vẫn là filter hợp lệ. Endpoint phải tiếp tục trả về số liệu lọc theo các chiều này khi được truyền — chỉ duy nhất `event` bị gỡ.

**Acceptance Criteria:**
- [ ] `GET /events/statistic?partner=<X>` trả về số liệu tổng hợp **theo partner X**.
- [ ] `GET /events/statistic?category=<C>` / `?fromAt=&toAt=` vẫn lọc đúng theo các chiều đó.
- [ ] `GET /events/statistic?event=<bất kỳ>&partner=<X>` cho kết quả giống `?partner=<X>` (event bị bỏ qua, các filter khác còn tác dụng).
...
**Description:**
Cache Redis của endpoint không còn key theo `event`. Cache key của Ambassador hiện theo `(event, partner, category)` (3 tham số) → bỏ chiều `event`, giữ `partner` + `category`. Sau deploy, cache cũ (đã chứa số liệu theo từng event) không được trả lại cho client.

**Acceptance Criteria:**
- [ ] Cache key của statistic không chứa thành phần `event` (chỉ theo partner/category/domain).
...
**Rationale:** Số liệu theo từng campaign (hoa hồng đã chi, độ phủ content) là thông tin business nhạy cảm — đây là mục tiêu chính của task.
...
- [ ] FE Ambassador hiện tại không cần sửa để hoạt động.
...
### EPIC-001: Siết quyền truy cập thống kê event trên API public (Ambassador)
...
- **STORY-2 (Verify):** As a developer, I want regression checks on `/events/user-newest`, FE Ambassador, và admin statistic so that gỡ filter không phá vỡ caller hiện hữu. (FR-004, NFR-002)
...
- **Public user / Ambassador (chưa login hoặc đã login):** xem block thống kê tổng hợp ở trang home Ambassador. Không được phép xem số liệu của 1 campaign cụ thể.
- **Đối tác (partner):** gọi qua `partner`/`category` filter, xem số liệu theo phạm vi mình. Hành vi giữ nguyên.
...
1. **Ambassador vào trang home** → FE gọi statistic (domain/partner/category) → BE trả số liệu tổng hợp → render block thống kê.
2. **Kẻ tấn công thử `?event=<ID>`** → BE bỏ qua `event` → trả số tổng hợp giống mọi người → không lấy được số riêng của campaign đó.
3. **Admin xem báo cáo campaign** → dùng API admin (ngoài phạm vi PRD) → vẫn xem chi tiết theo event.
...
- Redis cache key statistic (`GetKeyCacheStatisticEvent(event, partner, category)` — 3 tham số) — cần đổi format/purge, bỏ chiều `event`.
...
- Cần **verify FE Ambassador** không truyền `event` lên `/events/statistic` (tham chiếu vcreator: cả 2 FE vcreator đã verify KHÔNG truyền `event`; Ambassador cùng dòng FE nên nhiều khả năng tương tự — cần verify lại).
...
- Đụng các filter hợp lệ khác (`partner`, `category`, `fromAt/toAt`).
- Audit/siết các endpoint public khác.
...
2. Có cần verify lại FE Ambassador trên staging trước prod không?
3. Đồng bộ fix sang vcreator & T-Fluencers trong cùng đợt?
...
|---------|-----------|-------------------------|--------------------|
| EPIC-001 | Siết quyền truy cập thống kê event trên API public (Ambassador) | FR-001, FR-002, FR-003, FR-004 | 2 stories |
