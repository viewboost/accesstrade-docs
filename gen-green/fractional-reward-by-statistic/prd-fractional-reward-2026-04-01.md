...
**Thay đổi cốt lõi:** Tạo utility function `CashFloor` dùng chung, áp dụng tại tất cả nguồn sinh `reward.Cash` (by-statistic và milestone). Toàn bộ downstream (cashflow, user statistic, reconciliation, export, transfer) nhận giá trị đã integer nên **không cần thay đổi**. Khi cần đổi rounding logic sau này, chỉ sửa 1 function.
...
│     → CẦN pfloat.CashFloor() ở đây                             │
│                                                                 │
│  2. Milestone reward (CashMilestone gán thẳng):                 │
│     reward.Cash = schema.CashReward.CashMilestone               │
│     → Hiện tại admin nhập integer, nhưng tương lai có thể lẻ   │
│     → CẦN pfloat.CashFloor() cho safe                          │
│                                                                 │
...
### FR-002: Utility function `CashFloor` + áp dụng tại nguồn sinh cash

**Priority:** Must Have

**Description:**
Tạo function `CashFloor(v float64) float64` trong package `internal/util/pfloat` (đã có sẵn utility float). Function này wrap `math.Floor` — khi cần đổi rounding logic sau này chỉ sửa 1 chỗ. Áp dụng tại **tất cả nguồn sinh `reward.Cash`**:
- **By-statistic**: floor tổng `TotalCashLike + TotalCashComment + TotalCashView`
- **Milestone**: floor `CashMilestone` — hiện tại admin nhập integer nhưng sau này có thể setup lẻ

Toàn bộ downstream (cashflow, statistic, reconciliation, export, transfer) nhận giá trị đã integer nên tự động đúng, không cần thay đổi.

**Acceptance Criteria:**
- [ ] `pfloat.CashFloor(500.5)` = 500, `pfloat.CashFloor(2.1)` = 2, `pfloat.CashFloor(500.0)` = 500
- [ ] `CashPerView=0.5, views=1001` → `reward.Cash = pfloat.CashFloor(500.5) = 500`
- [ ] `CashMilestone=50000` → `reward.Cash = pfloat.CashFloor(50000.0) = 50000` (backward compatible)
- [ ] `CashMilestone=0.5` (tương lai) → `reward.Cash = pfloat.CashFloor(0.5) = 0` (safe)
- [ ] Downstream tự đúng: cashflow value = integer, user statistic = integer, export = integer

**Files cần cập nhật:**

| File | Dòng | Thay đổi |
|------|------|----------|
| `internal/util/pfloat/float.go` | (new) | Thêm `func CashFloor(v float64) float64 { return math.Floor(v) }` |
| `internal/service/event_schema.go` | L284 | `reward.Cash = pfloat.CashFloor(...)` trong `UpdateRewardTypeByStatisticContent()` |
| `internal/service/event_schema.go` | L689 | `reward.Cash = pfloat.CashFloor(...)` trong `CheckPassSchemaWithContent()` |
| `internal/service/event_schema.go` | L104 | `Cash: pfloat.CashFloor(s.CashReward.CashMilestone)` — milestone in `RunCheckPassSchemaStatistic` |
| `internal/service/event_schema.go` | L443 | `Cash: pfloat.CashFloor(schema.CashReward.CashMilestone)` — milestone in `CheckPassSchemaWithContent` |
| `pkg/public/service/schedule.go` | L925 | `Cash: pfloat.CashFloor(schema.CashReward.CashMilestone)` — milestone in schedule job |

**Tại sao không cần floor ở downstream?**

Vì `reward.Cash` là **nguồn gốc duy nhất** sinh ra cash. Sau khi floor ở đây:
...
| File | Dòng | Thay đổi |
|------|------|----------|
| `pkg/admin/service/reconciliation_running.go` | L226 | `pfloat.CashFloor(totalCashPending) != pfloat.CashFloor(item.TotalCash)` |
...
│                                                              │
│  Nguyên tắc: Floor TẠI NGUỒN qua pfloat.CashFloor(),       │
│  không floor downstream.                                     │
...
│  3. reward.Cash (by-statistic): pfloat.CashFloor() ↓        │
│  4. reward.Cash (milestone): pfloat.CashFloor() ↓           │
│  5. Downstream (cashflow, statistic, reconciliation,         │
│     export, transfer): KHÔNG CẦN floor — nhận integer        │
│  6. Tax: giữ nguyên math.Round() (convention thuế)           │
...
**Acceptance Criteria:**
- [ ] `reward.Cash` (by-statistic và milestone) luôn là số nguyên sau `pfloat.CashFloor()`
...
### EPIC-002: CashFloor Utility + Reward Floor

**Description:** Tạo `pfloat.CashFloor()` utility function, áp dụng tại tất cả nguồn sinh `reward.Cash` (by-statistic 2 chỗ + milestone 3 chỗ).
...
| EPIC-002: CashFloor Utility + Reward Floor | FR-002 | NFR-001, NFR-003 | 1 | Must |
...
|---|---------|---------|-------|
| 1 | Cần floor ở bao nhiêu chỗ? | **6 chỗ** qua `pfloat.CashFloor()` | By-statistic (2) + milestone (3) + reconciliation (1). Tất cả gọi qua 1 utility function. Downstream không cần. |
...
| 5 | Milestone reward có cần floor? | **Có** — qua `pfloat.CashFloor()` | Hiện tại admin nhập integer nên không ảnh hưởng (`CashFloor(50000.0) = 50000`). Nhưng tương lai admin có thể setup lẻ, dùng `CashFloor` sẵn cho safe. |
...
| Ai đó sau này thêm nguồn cash mới không floor | Medium | Reconciliation `!=` fix (FR-003) sẽ catch. `pfloat.CashFloor()` dễ grep/audit. |
...
**Total files cần thay đổi: 4 (Must Have) + 1 (Should Have)**
...
| 3 | `internal/service/event_schema.go` | `pfloat.CashFloor(reward.Cash)` — 2 chỗ by-statistic + 2 chỗ milestone | S | Must |
| 4 | `pkg/public/service/schedule.go` | `pfloat.CashFloor(CashMilestone)` — 1 chỗ milestone | S | Must |
| 5 | `pkg/admin/service/reconciliation_running.go` | `pfloat.CashFloor()` cả 2 vế trước `!=` | S | Should |

**Tổng: 7 code locations across 5 files | Tất cả gọi qua 1 utility function `pfloat.CashFloor()`**
