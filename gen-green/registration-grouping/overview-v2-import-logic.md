...
## 1.5. Bảng tra nhanh tình huống → action

> **Quy ước viết tắt:**
> - **DSNV** = Bảng danh sách nhân viên chính thức HR đã import từ trước (collection MongoDB: `employee-registries`)
> - **User** = Tài khoản người dùng đăng ký trên app Gen-Green (collection MongoDB: `users`)
> - **File** = File Excel HR đang upload lần này (parse thành các `RegistryRow` in-memory, chưa lưu DB)
> - ✅ = có/khớp · ❌ = không có/không khớp · `*` = không quan trọng · `—` = không áp dụng

### Mapping fields được dùng để check

| Tên gọi trong bảng | Collection | Field MongoDB |
|---|---|---|
| Mã NV trong DSNV | `employee-registries` | `employeeCode` |
| SĐT trong DSNV | `employee-registries` | `phone` |
| Đơn vị DSNV | `employee-registries` | `workplaceUnitName` (fallback `workplaceName`) |
| Trạng thái DSNV | `employee-registries` | `status` (`active` / `terminated`) |
| Mã user | `users` | `employeeCode` |
| SĐT user | `users` | `phone.number` (normalize về `0xxxxxxxxx` trước khi compare) |
| Đơn vị user | `users` | `workplaceUnitName` (KHÔNG dùng để check transferred — có thể stale) |
| Trạng thái user | `users` | `staffStatus` (`pending` / `verified` / `rejected` / `pending_removal`) |
| Mã NV file | (in-memory) | `RegistryRow.EmployeeCode` |
| SĐT file | (in-memory) | `RegistryRow.Phone` (đã normalize) |
| Đơn vị file | (in-memory) | `RegistryRow.WorkplaceName` |

### Bảng A — Match từng dòng file vào DSNV + User

| # | Mã NV trong DSNV | SĐT có user | Mã user vs Mã file | SĐT user vs SĐT DSNV | Đơn vị DSNV vs File | Trạng thái user | Action | Ý nghĩa |
|---|---|---|---|---|---|---|---|---|
| 1 | ❌ | ❌ | — | — | — | — | `new_record` | Thêm mới hoàn toàn vào DSNV |
| 2 | ❌ | ✅ | ✅ | — | — | pending | `auto_verified` | User khai trước HR — duyệt + insert DSNV |
| 3 | ❌ | ✅ | ❌ | * | — | * | `cancelled_mismatch` | Xung đột mã NV (`code_mismatch`) |
| 4 | ❌ | ✅ | ✅ | ❌ | — | * | `cancelled_mismatch` | Xung đột SĐT (`phone_mismatch`) |
| 5 | ❌ | ✅ | ❌ | ❌ | — | * | `new_record` | Lệch cả mã + SĐT — coi như NV mới, không link user |
| 6 | ✅ | ❌ | — | — | giống | — | `unchanged` | DSNV y hệt file, không cần làm gì |
| 7 | ✅ | ❌ | — | — | * | — | `registry_updated` | HR đính chính SĐT/đơn vị (chưa user nào claim mã) |
| 8 | ✅ (terminated) | ❌ | — | — | * | — | `registry_updated` | **Hồi sinh** mã NV đã nghỉ → status = active |
| 9 | ✅ | ✅ | ✅ | ✅ | giống | pending | `auto_verified` | User pending + đúng người → duyệt |
| 10 | ✅ | ✅ | ✅ | ✅ | giống | verified/rejected | `unchanged` | Đã verified rồi, không cần làm gì |
| 11 | ✅ | ✅ | ✅ | ✅ | **khác** | * | `transferred` | **Điều chuyển** — đổi đơn vị làm việc |
| 12 | ✅ | ✅ | ❌ | * | — | * | `cancelled_mismatch` | Xung đột mã NV (`code_mismatch`) |
| 13 | ✅ | ✅ | ✅ | ❌ | — | * | `cancelled_mismatch` | Xung đột SĐT (`phone_mismatch`) |

### Bảng B — Action sinh ngoài match per-row

| Action | Khi nào sinh | Điều kiện | Apply làm gì |
|---|---|---|---|
| `missing_from_file` | Sau khi match xong (chỉ khi admin BẬT toggle "Rà soát nghỉ việc") | DSNV có mã NV X (status=active, trong scope), file KHÔNG có | Schedule terminate user sau 7 ngày grace period |
| `invalid` | Lúc parse Excel | Row sai format (mã rỗng, SĐT invalid, đơn vị không có trong master, etc.) | Skip, hiển thị lỗi cho admin |

### Giải thích cách so sánh (đối xứng tự nhiên do hash join)

Engine match dùng hash join để optimize (xem `internal/service/registry_match.go`):
- Hash `registryMap` key = `employee-registries.employeeCode` → nếu lookup `RegistryRow.EmployeeCode` ra ✅, đã ngầm chứng minh `DSNV.employeeCode = file.employeeCode`
- Hash `userMap` key = `users.phone.number` (đã normalize) → nếu lookup `RegistryRow.Phone` ra ✅, đã ngầm chứng minh `user.phone.number = file.phone`

Còn lại 2 vế cần verify (code thực tế trong `buildChange`):
```go
codeMatch  := existingUser.EmployeeCode == row.EmployeeCode  // users.employeeCode vs file
phoneMatch := userPhone == registryPhone                     // users.phone.number vs employee-registries.phone
```

- **`users.employeeCode`** vs **`RegistryRow.EmployeeCode`** → user có claim đúng mã trong file không
- **`users.phone.number`** vs **`employee-registries.phone`** → SĐT user dùng có khớp SĐT HR ghi cho mã đó không

→ Engine ngầm verify đủ 4 vế (`DSNV ↔ User ↔ File`), chỉ asymmetric trong code do hash optimization.

### Reason key cho `cancelled_mismatch`

Lưu vào `users.staffRejectReason` (text Vietnamese map từ key bằng `MapMismatchReason`):

| Reason key (DB) | Text user-friendly | Ý nghĩa |
|---|---|---|
| `code_mismatch` | "Số điện thoại bạn cung cấp thuộc về một mã nhân viên khác" | SĐT user đúng, mã user khai sai |
| `phone_mismatch` | "Mã nhân viên bạn cung cấp thuộc về một số điện thoại khác" | Mã user đúng, SĐT user dùng sai |

### Field bị thay đổi khi apply mỗi action

| Action | Collection bị update | Field chính bị set/reset |
|---|---|---|
| `auto_verified` | `users` | `staffStatus = verified`, `staffVerifiedAt`, `workplaceBrandCode/Name`, `workplaceCompanyCode/Name`, `workplaceUnitCode/Name` |
| | `employee-registries` | `genGreenUserId`, `lastSeenImportId` |
| `transferred` | `users` | `workplaceBrandCode/Name`, `workplaceCompanyCode/Name`, `workplaceUnitCode/Name` |
| | `employee-registries` | `workplaceName`, `workplaceUnitName/UnitCode/CompanyName/CompanyCode/BrandName/BrandCode`, `lastSeenImportId` |
| `cancelled_mismatch` | `users` | `staffStatus = rejected`, `staffRejectReason`, clear `employeeCode` + workplace fields |
| | `employee-registries` | `lastSeenImportId` (KHÔNG link `genGreenUserId`) |
| `new_record` | `employee-registries` | INSERT mới với `status = active`, `importedAt`, `importId`, `lastSeenImportId` |
| `registry_updated` | `employee-registries` | `phone` / `workplaceName` / `status` (revive terminated → active), `lastSeenImportId` |
| `unchanged` | `employee-registries` | Chỉ `lastSeenImportId` |
| `missing_from_file` | `users` | `staffRemovalScheduledAt = now + 7 ngày` (sau 7 ngày cron daily 00:00 mới gỡ staff tag) |
| `invalid` | (không update) | Chỉ hiển thị lỗi, skip apply |
